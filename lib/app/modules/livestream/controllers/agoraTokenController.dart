...
class AgoraTokenController extends GetxController {
  ...
  Future<Map<String, dynamic>?> requestAgoraToken({
    ...
    final bool cacheStillFresh = lastSuccessAt != null &&
  ...
  Future<Map<String, dynamic>?> _performTokenRequest({
    ...
    try {
      ...
      if (!forceRefresh) agoraToken.clear();
      liveLog(
        '⚠️ Failed to generate agora token: '
      ...
      if (!forceRefresh) agoraToken.clear();
      liveLog('❌ Agora token Dio error => ${e.response?.data ?? e.message}');
      return null;
    } catch (e) {
      if (!forceRefresh) agoraToken.clear();
  ...
  String getTokenStringFrom(Map<String, dynamic> source) {
    ...
    final dynamic token = source['token'] ??
        source['rtc_token'] ??
        source['agora_token'] ??
        data['token'] ??
        data['rtc_token'] ??
  ...
  int getExpiresInSecondsFrom(
    ...
    final Map<String, dynamic> data = _nestedData(source);
    final dynamic rawSeconds = source['expires_in'] ??
    ...
    final dynamic rawExpiry = source['expires_at'] ??
